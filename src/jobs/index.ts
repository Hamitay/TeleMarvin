...
export class Jobs {
  ...
  async nextWeeklySession(): Promise<void> {
    const sessions =
      await this.#weeklySessionService.getWeeklySessionsOfTheDay();
    sessions.forEach((session) => {
      try {
        ...
        console.error("Error sending pool for weekly session");
        console.error(error);
      }
    });
  }
  ...
  async nextSessionJob(): Promise<void> {
    ...
    sessions.forEach((session) => {
      try {
        ...
        console.error("Error sending pool for session");
        console.error(error);
      }
    });
  }
